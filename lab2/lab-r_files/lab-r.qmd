...
args(theme)
```
The big groups here are `axis`, `legend`, `panel` (facets), `plot` (the broader plot elements), and `strip` (the titles of the facets). 
Each of these elements can be customized. Details are available using `?theme`.
